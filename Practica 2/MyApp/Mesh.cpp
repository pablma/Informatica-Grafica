...
{
	Mesh* m = new Mesh();
	m->type = GL_TRIANGLE_FAN;	//v0, v1, v2
	m->numVertices = 5;			//v0, v2, v3
								//v0, v3, v4
	m->vertices = new dvec3[m->numVertices];
...
{
	Mesh* m = new Mesh();
	m->type = GL_TRIANGLE_STRIP;	//v0, v1, v2
	m->numVertices = 10;			//v1, v3, v2
									//v2, v3, v4
	m->vertices = new dvec3[m->numVertices];
	...
	m->vertices[8] = dvec3(l / 2, -l / 2, -l / 2);	//Es el mismo que v0
	m->vertices[9] = dvec3(l / 2, l / 2, -l / 2);	//Es el mismo que v1
...
Mesh* Mesh::generateRectangleTex(GLdouble w, GLdouble h, GLint divW, GLint divH)
{
	...
	m->texCoords[1] = dvec2(1 + divW, 0);
	m->texCoords[2] = dvec2(0, 1 + divH);
	m->texCoords[3] = dvec2(1 + divW, 1 + divH);
...
Mesh* Mesh::generateContCuboTex(GLdouble l)
{
	...
	//m->texCoords[6] = dvec2(1, 0);
	//m->texCoords[7] = dvec2(1, 1);

	//m->texCoords[8] = dvec2(0, 0);
	//m->texCoords[9] = dvec2(0, 1);
	...
	m->texCoords[7] = dvec2(0, 1);

	m->texCoords[8] = dvec2(1, 0);
	m->texCoords[9] = dvec2(1, 1);
